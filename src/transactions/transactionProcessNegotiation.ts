...
export const transitions = {
  ...
  CUSTOMER_REJECT_FROM_UPDATE_PENDING:
    'transition/customer-reject-from-update-pending',
  PROVIDER_WITHDRAW_FROM_UPDATE_PENDING:
  ...
  AUTO_CANCEL_FROM_CHANGES_REQUESTED:
    'transition/auto-cancel-from-changes-requested',
  OPERATOR_CANCEL_FROM_CHANGES_REQUESTED:
...
export interface TransitionWithOfferInSubunitsRecord extends TransitionRecord {
  offerInSubunits?: number;
}
...
// ];
const makeOfferTransitions: TransitionType[] = [
...
const counterOfferTransitions: TransitionType[] = [
  transitions.CUSTOMER_MAKE_COUNTER_OFFER,
  transitions.PROVIDER_MAKE_COUNTER_OFFER,
];

// transitions that revoke a counter offer
const revokeCounterOfferTransitions: TransitionType[] = [
...
const offerTransitions: TransitionType[] = [
...
// };
const filterOfferTransitions = (
  txs: TransitionRecord[],
  offers: TransitionType[],
): TransitionRecord[] => {
  return txs.filter(t => offers.includes(t.transition));
};
...
//   const pickedTransitions = filterOfferTransitions(
//     transitions,
//     offerTransitions,
//   );
...
export const isValidNegotiationOffersArray = (
  txs: TransitionRecord[],
  offers: NegotiationOffer[],
): boolean => {
  const picked = filterOfferTransitions(txs, offerTransitions);
  if (!Array.isArray(offers) || offers.length !== picked.length) return false;

  return offers.every(
    (offer, i) =>
      offer.transition === picked[i].transition && offer.by === picked[i].by,
  );
};
 ...
 * @returns {boolean}
 */
export const isNegotiationState = (state?: string): boolean => {
  if (!state) return false;
  const unprefixed = state.includes('/') ? state.split('/')[1] : state;
  return (
    unprefixed === states.OFFER_PENDING ||
    unprefixed === states.CUSTOMER_OFFER_PENDING
  );
};
 ...
 */
export const getTransitionsWithMatchingOffers = (
  txs: TransitionRecord[],
  offers: NegotiationOffer[],
): TransitionWithOfferInSubunitsRecord[] => {
  if (!isValidNegotiationOffersArray(txs, offers)) {
    return txs as TransitionWithOfferInSubunitsRecord[];
  }

  const result: TransitionWithOfferInSubunitsRecord[] = [];
  let i = 0;

  for (const tx of txs) {
    const withOffer: TransitionWithOfferInSubunitsRecord = {...tx};
    if (offerTransitions.includes(tx.transition)) {
      withOffer.offerInSubunits = offers[i]?.offerInSubunits;
      i++;
    }
    result.push(withOffer);
  }
...
export const isRelevantPastTransition = (
  transition: TransitionType,
): boolean => {
  const relevantTransitions: TransitionType[] = [
    transitions.MAKE_OFFER,
    transitions.MAKE_OFFER_AFTER_INQUIRY,
  ...
  return relevantTransitions.includes(transition);
};
...
export const isCustomerReview = (transition: TransitionType): boolean => {
  const customerReviews: TransitionType[] = [
    transitions.REVIEW_1_BY_CUSTOMER,
    transitions.REVIEW_2_BY_CUSTOMER,
  ];
  return customerReviews.includes(transition);
};
// Processes might be different on how reviews are handled.
// Default processes use two-sided diamond shape, where either party can make the review first
export const isProviderReview = (transition: TransitionType): boolean => {
  const providerReviews: TransitionType[] = [
    transitions.REVIEW_1_BY_PROVIDER,
    transitions.REVIEW_2_BY_PROVIDER,
  ];
  return providerReviews.includes(transition);
};
...
// enough.
export const isPrivileged = (transition: TransitionType): boolean => {
  const privilegedTransitions: TransitionType[] = [
    transitions.MAKE_OFFER,
    transitions.MAKE_OFFER_AFTER_INQUIRY,
    transitions.MAKE_OFFER_FROM_REQUEST,
    transitions.CUSTOMER_MAKE_COUNTER_OFFER,
  ...
  return privilegedTransitions.includes(transition);
};

// Check when transaction is completed (offer is received and review notifications sent)
export const isCompleted = (transition: TransitionType): boolean => {
  const txCompleted: TransitionType[] = [
    transitions.ACCEPT_DELIVERABLE,
    transitions.AUTO_ACCEPT_DELIVERABLE,
    transitions.OPERATOR_ACCEPT_DELIVERABLE,
    transitions.REVIEW_1_BY_CUSTOMER,
  ...
  return txCompleted.includes(transition);
};
...
export const isRefunded = (transition: TransitionType): boolean => {
  const refunded: TransitionType[] = [
    transitions.EXPIRE_PAYMENT,
    transitions.AUTO_CANCEL,
    transitions.OPERATOR_CANCEL,
    transitions.OPERATOR_CANCEL_FROM_DELIVERED,
...
export const statesNeedingProviderAttention: StateType[] = [
  states.QUOTE_REQUESTED,
  states.CUSTOMER_OFFER_PENDING,
  states.OFFER_ACCEPTED,
  states.CHANGES_REQUESTED,
...
export const statesNeedingCustomerAttention: StateType[] = [
  states.OFFER_PENDING,
  states.DELIVERED,
];
