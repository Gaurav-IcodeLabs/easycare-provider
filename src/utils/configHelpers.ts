...
import {getSupportedProcessesInfo} from '../transactions';
import {
  getCategoriesList,
...
const getMaxDepth = (depth: number, subcategories: any[]) =>
  subcategories.length ? Math.max(...subcategories) : depth;
const createArray = (length: string) =>
...
const mergeCurrency = (hostedCurrency, defaultCurrency) => {
  const currency = defaultCurrency || hostedCurrency;
...
const mergeUserConfig = (hostedConfig, defaultConfigs) => {
  ...
  ).filter(elm => elm.userType === 'customer');
...
export function transformCategoryData(arr) {
  // Create a map to easily reference each item by id
  const map = {};

  // Initialize the map with all items, replacing 'label' with 'name'
  arr.forEach(item => {
    map[item.id] = {
      id: item.id,
      name: item.label, // Change 'label' to 'name'
      subcategories: [], // Initialize subcategories as empty array
    };
  });

  // Array to store the final transformed result
  const result = [];

  // Iterate through the array again to build the parent-child relationships
  arr.forEach(item => {
    if (item.parentId) {
      // If the item has a parentId, push it into the parent's subcategories
      map[item.parentId].subcategories.push({
        id: map[item.id].id,
        name: map[item.id].name,
        subcategories: map[item.id].subcategories, // Subcategories array stays empty for children without their own children
      });
    } else {
      // If there's no parentId, it's a top-level category
...
export function transformDealCategoryData(arr) {
  // Create a map to reference each item by id
  const map = {};
  ...
  arr.forEach(item => {
    map[item.id] = {
      id: item.id,
      name: item.label, // Change 'label' to 'name'
      description: item.description || '', // Add description if exists
      color: item.color || '', // Add color if exists
  ...
  arr.forEach(item => {
    if (item.parentId) {
      ...
      map[item.parentId].subcategories.push({
        id: map[item.id].id,
        name: map[item.id].name,
        description: map[item.id].description, // Keep the description
        color: map[item.id].color, // Keep the color
        subcategories: [], // Child subcategories are empty
      });
    } else {
      // If there's no parentId, it's a top-level category
      result.push(map[item.id]);
...
export async function fetchCategoriesAndDeals() {
  try {
    // Call both getCategoriesList and getDealCategoriesList in parallel
    const [categoriesResult, dealCategoriesResult, interestsResult] =
      await Promise.all([
        getCategoriesList(), // Fetch the categories list
        getDealCategoriesList(), // Fetch the deal categories list
        getInterestsList(),
      ]);

    // Process the fetched data using the provided transformation functions
    const categoriesList =
      categoriesResult.length > 0
        ? transformCategoryData(categoriesResult)
        : [];

    const dealCategories =
      dealCategoriesResult.length > 0
        ? transformDealCategoryData(dealCategoriesResult)
        : [];
    const interestsList = interestsResult.length > 0 ? interestsResult : [];
    // Return or process both lists here
    return {categoriesList, dealCategories, interestsList};
  } catch (error) {
    // Handle any error that occurred in the Promise.all block
    console.error('Error fetching categories or deals:', error);
    // You can return a default fallback or handle the error in other ways
    return {categoriesList: [], dealCategories: []};
  }
}
